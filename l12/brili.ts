...
function evalInstr(instr: bril.Instruction, state: State): Action {
  ...
  if (tracing && traceCount < ttlTrace && instr.op !== "call" && instr.op !== "ret" && instr.op !== "br") {
    const line = JSON.stringify(instr) + "\n"
    // Deno.writeSync(encoder.encode(line));
    Deno.writeTextFile(TRACE_FILE, line, { append: true, create: true });
    traceCount++;
  }
  if (instr.op === "call" || instr.op === "ret") {
    tracing = false
  }
  ...
  case "br": {
    ...
    if (cond) {
      if (tracing && traceCount < ttlTrace) {
        let new_instr = { ...instr, labels: [getLabel(instr, 0)] };
        const line = JSON.stringify(new_instr) + "\n";
        Deno.writeTextFile(TRACE_FILE, line, { append: true, create: true });
        traceCount++;
      }
      return {"action": "jump", "label": getLabel(instr, 0)};
      ...
      if (tracing && traceCount < ttlTrace) {
        let negation = {op: "not", args: instr.args, dest: instr.args[0] + "_neg", type: "bool"};
        let new_instr = { ...instr, labels: [getLabel(instr, 1)], args: [instr.args[0] + "_neg"] };
        const line1 = JSON.stringify(negation) + "\n";
        const line2 = JSON.stringify(new_instr) + "\n";
        Deno.writeTextFile(TRACE_FILE, line1, { append: true, create: true });
        Deno.writeTextFile(TRACE_FILE, line2, { append: true, create: true });
        traceCount++;
      }
...
async function main() {
  try {
    ...
    Deno.writeTextFileSync(TRACE_FILE, "", { create: true, truncate: true });
    evalProg(prog);
  }
  catch(e) {
